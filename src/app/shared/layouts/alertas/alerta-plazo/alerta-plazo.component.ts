...
@Component({
  ...
  imports: [
    ...
    MenuModule,
    CmpLibModule,
    NgxCfngCoreLayoutHeaderComponent,
    tiempoSegundos,
    textoTiempoSegundos,
    NgIf,
  ],
  templateUrl: './alerta-plazo.component.html',
  styleUrl: './alerta-plazo.component.scss',
  providers: [DialogService],
})
export class AlertaPlazoComponent implements OnInit{
  @Input()
  public eventoCerrar!: () => void;
  @Input()
  public listaAlertasPlazoPorAtender: Alerta[] = [];
  @Input()
  public listaAlertasPlazoAtendidos: Alerta[] = [];
  ...
  constructor(
    private router: Router, protected iconUtil: IconUtil,
  ...
  private actualizarAlertas(alerta: Alerta) {
    this.alertaService.actualizarAlerta(alerta).subscribe({
      next: (data) => {
        this.listaAlertasPlazoAtendidos.push(this.listaAlertasPlazoPorAtender.filter(
          (alerta) => alerta.id === data.id
        )[0]);
        this.listaAlertasPlazoAtendidos.sort((a, b) => {
          const fechaA = new Date(a.fechaCreacion).getTime();
          const fechaB = new Date(b.fechaCreacion).getTime();
          return fechaB - fechaA;
        });
        this.listaAlertasPlazoPorAtender = this.listaAlertasPlazoPorAtender.filter(
          (alerta) => alerta.id !== data.id
        );
  ...
  protected verAlertas() {
    this.router.navigate(['app/bandeja-alertas'], { queryParams: { 'tipo-alerta': '0' } }).then(() => {
