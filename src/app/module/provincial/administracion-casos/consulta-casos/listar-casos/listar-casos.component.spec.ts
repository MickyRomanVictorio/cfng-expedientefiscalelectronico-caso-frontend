import { ComponentFixture, TestBed } from '@angular/core/testing';
import { ListarConsultaCasosComponent } from './listar-casos.component';
import { provideHttpClient } from '@angular/common/http';

describe('ListarConsultaCasosComponent', () => {
  let component: ListarConsultaCasosComponent;
  let fixture: ComponentFixture<ListarConsultaCasosComponent>;

  beforeEach(async () => {
    const mockToken = JSON.stringify({token:'Bearer eyJhbGciOiJIUzUxMiJ9..XP8ZiMZSizPtvJ-3LLKxsAhKJYgR50A5jZU-Z9T1wv9SgDKzgag7R-27kG185HgdbDVIHDaJ73-bYfV_Mz2J3g'});
    spyOn(sessionStorage, 'getItem').and.callFake((key: string) => {
      return mockToken;
    });
    //
    await TestBed.configureTestingModule({
      imports: [
        ListarConsultaCasosComponent
      ],
      providers: [
        provideHttpClient()
      ],
    })
    .compileComponents();

    fixture = TestBed.createComponent(ListarConsultaCasosComponent);
    component = fixture.componentInstance;
    fixture.detectChanges();
  });

  it('should create', () => {
    expect(component).toBeTruthy();
  });
});
