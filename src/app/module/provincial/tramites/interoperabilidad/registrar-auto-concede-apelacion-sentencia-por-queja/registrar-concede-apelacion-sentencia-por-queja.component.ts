...
@Component({
  standalone: true,
  selector: 'app-registrar-concede-apelacion-sentencia-por-queja',
  templateUrl: './registrar-concede-apelacion-sentencia-por-queja.component.html',
  styleUrls: ['./registrar-concede-apelacion-sentencia-por-queja.component.scss'],
  imports: [
    NgClass,
    FormsModule,
    ReactiveFormsModule,
...
export class RegistrarConcedeApelacionSentenciaPorQuejaComponent implements OnInit, OnDestroy {
  @Input() idCaso: string = '';
  ...
  private readonly resolucionAutoConcedeApelacionSentenciaPorQuejaService = inject(ResolucionAutoConcedeApelacionSentenciaPorQuejaService);
      ...
      next: (resp) => {
        if (resp === 'confirm') {
          ...
          this.suscripciones.push(
            this.resolucionAutoConcedeApelacionSentenciaPorQuejaService
  ...
  private validarElevar(): Promise<void> {
    return new Promise((resolve) => {
      this.suscripciones.push(
        this.resolucionAutoConcedeApelacionSentenciaPorQuejaService.validaElevar(this.idActoTramiteCaso).subscribe({
          next: (data: any) => {
  ...
  private eventoRegistrarResolucion(): void {
    ...
    this.suscripciones.push(
      this.resolucionAutoConcedeApelacionSentenciaPorQuejaService.registrarTramite(request).subscribe({
  ...
  public openModalSujetos(tipoModal: number): void {
    ...
    ref = this.dialogService.open(RegistrarAutoConcedeApelacionSentenciaPorQuejaModalComponent, dataModal);
