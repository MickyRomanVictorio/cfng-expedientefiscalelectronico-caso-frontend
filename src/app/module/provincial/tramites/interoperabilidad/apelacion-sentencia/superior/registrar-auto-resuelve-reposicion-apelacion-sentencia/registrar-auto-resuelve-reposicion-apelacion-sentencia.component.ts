...
@Component({
  ...
  templateUrl: './registrar-auto-resuelve-reposicion-apelacion-sentencia.component.html',
  styleUrls: ['./registrar-auto-resuelve-reposicion-apelacion-sentencia.component.scss'],
  imports: [
    NgClass,
    FormsModule,
    ReactiveFormsModule,
...
export class RegistrarAutoResuelveReposicionApelacionSentenciaComponent implements OnInit, OnDestroy {
  @Input() idCaso: string = '';
  ...
  protected mostrarCamposAgenda: boolean = false;
        ...
        .subscribe({
          next: (data) => {
            this.formularioAutoResuelve.patchValue({
              ...
              chkAgenda: data.chkAgenda || false,
              fechaAudiencia: data.fechaAudiencia || null,
              idTipoReunion: data.idTipoReunion || null,
              urlReunion: data.urlReunion || null,
              lugar: data.lugar || null,
              idDistritoJudicial: data.idDistritoJudicial || null
            });

            this.mostrarCamposAgenda = data.chkAgenda || false;
            ...
            if (data.idDistritoJudicial && data.idEntidad) {
              this.listarJuzgadosPazLetrado(data.idDistritoJudicial, () => {
                this.formularioAutoResuelve.patchValue({
  ...
  private registrarTramite(): void {
    ...
    const request = datosForm.chkAgenda
      ? { ...datosForm, idActoTramiteCaso: this.idActoTramiteCaso }
      : {
        idActoTramiteCaso: this.idActoTramiteCaso,
        fechaNotificacion: datosForm.fechaNotificacion,
        observaciones: datosForm.observaciones,
        chkAgenda: datosForm.chkAgenda
      };
  ...
  public abrirModalSujetos(): void {
    let dataModal = {
      ...
      data: {
        ...
        configuracion: {
          titulo: 'Seleccionar resultado del recurso de desistimiento de apelación de sentencia',
          subtitulo: 'Seleccione los sujetos procesales a los que se le ha declarado fundado y/o infundado el recurso de desistimiento',
          mensaje_informativo: {
            mostrar: false,
            descripcion: null,
  ...
  /* NECESARIO PARA AGENDA */
  changeAgendaAudiencia(event: CheckboxChangeEvent) {
    this.mostrarCamposAgenda = event.checked;
  ...
  protected listarJuzgadosPazLetrado(idDistritoJudicial: number, callback?: () => void): Subscription {
    const subscription = this.maestroService
  ...
  protected readonly ID_N_TIPO_ACTIV_AGENDA = ID_N_TIPO_ACTIV_AGENDA;
}
