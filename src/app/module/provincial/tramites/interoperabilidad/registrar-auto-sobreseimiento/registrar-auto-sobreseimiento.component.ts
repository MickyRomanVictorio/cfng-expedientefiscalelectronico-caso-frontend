...
import { ResolucionAutoSobreseimientoService } from '@core/services/provincial/tramites/comun/preparatoria/resolucion-auto-sobreseimiento.service';
...
@Component({
  selector: 'app-registrar-auto-sobreseimiento',
  standalone: true,
  imports: [
    NgClass,
    FormsModule,
  ...
  templateUrl: './registrar-auto-sobreseimiento.component.html',
  styleUrl: './registrar-auto-sobreseimiento.component.scss',
})
export class RegistrarAutoSobreseimientoComponent implements OnInit, OnDestroy {
  @Input() idCaso: string = '';
  ...
  private readonly autoSobreseimientoService = inject(
    ResolucionAutoSobreseimientoService
  );
  ...
  get cantidadCaracteresObservacion(): number {
    return (
      this.formularioAutoReparatorio.get('observaciones')?.value?.length ?? 0
    );
  }
  ...
  private obtenerDetalleActoTramiteCaso() {
    ...
    this.obtenerAutoSobreseimiento();
  }
    ...
    dialog.subscribe({
      next: (resp: CfeDialogRespuesta) => {
        if (resp === CfeDialogRespuesta.Confirmado) {
          this.registrarAutoSobreseimiento();
        }
      },
    });
  }
  ...
  private obtenerAutoSobreseimiento(): void {
    this.suscripciones.push(
      this.autoSobreseimientoService
        .obtenerAutoSobreseimiento(this.idActoTramiteCaso)
        .subscribe({
          next: (resp: AutoSobreseimiento) => {
  ...
  private registrarAutoSobreseimiento(): void {
    this.suscripciones.push(
      this.autoSobreseimientoService
        .registrarAutoSobreseimiento(this.formularioAutoReparatorio.value)
        .subscribe({
