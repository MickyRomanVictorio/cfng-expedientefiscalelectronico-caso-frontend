...
export class CustomFormService {
  ...
  closeModal: eventModal | null = null;
  openModal: eventModal | null = null;
  ampliarPlazo: eventModal | null = null;
  ...
  createFormulario() {
    return  this.fb.group({
        ...
        [field.plazo]: [{ value: null, disabled: false }, [ Validators.required, Validators.minLength(1), Validators.maxLength(3), Validators.pattern(/^[0-9]*$/)]],
  ...
  plazoMaximo() {
    return (form:any) => {
      ...
      if(uniMedida && plazo && complejidad) {
        const { isPasoElmaximo, mensaje } = calcularPlazoMaximoUtil(Number(complejidad), Number(canMax), Number(uniMedida), Number(plazo)).build();
