...
export class AmpliarPlazoComponent implements OnInit {
  ...
  fechaFinInvPreliminar() {
    console.log("Mostrar calificacion: ", JSON.stringify( this.aps.calificarCaso) )
  ...
  public ampliar(): void {
    console.log("AMPLIAR PLAZO: ", this.aps.form!.value)
  ...
  obtenerValorMaxPlazo() {
    ...
    console.log("OBTENER PLAZO MAXIMO 1 ")
    const idTipoUnidad = this.aps.form!.get(this.fd.unidadMedida)!.value;
    console.log("OBTENER PLAZO MAXIMO: ", idTipoUnidad)
  ...
  private handlerListarUnidadMedida() {
    return (resp:any) => this.unidadesMedidas = resp.filter((e:any) => e.id !== 1).map((item:any) => ({
      value: item.id,
      label: item.nombre
    }))
  }
  ...
  private handlerValorMaxPlazo() {
    ...
    return (result:any) => {
      console.log("OBTENER PLAZO MAXIMO 2 ", result)
