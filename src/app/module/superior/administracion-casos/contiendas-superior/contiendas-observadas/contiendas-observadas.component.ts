...
@Component({
  standalone: true,
  selector: 'app-contiendas-observadas',
  templateUrl: './contiendas-observadas.component.html',
  styleUrls: ['./contiendas-observadas.component.scss'],
  imports: [
    CalendarModule,
    CheckboxModule,
    CmpLibModule,
...
export class ContiendasObservadasComponent implements OnInit {
  ...
  protected contiendasObservadasList: ContiendasCompetencias[] = []
  public contiendasObservadasFiltrados: ContiendasCompetencias[] = []
  ...
  ngOnInit(): void {
    ...
    this.opcionesMenu = (data: any) => [
      ...
      },
      {
        ...
        icon: 'file-search-icon',
        command: () => {
          this.mostrarAsuntoObservaciones(data.idCaso, data.codigoCaso)
  ...
  private obtenerLeyenda(contiendasObservadasFiltrados:any): void {
    var nroCasosDentroPlazo = 0;
    var nroCasosPorVencer = 0;
    var nroCasosFueraplazo = 0;

    contiendasObservadasFiltrados.forEach( (caso:any) => {
  ...
  public buscarObservadosPorMeses() {
  ...
  protected listarContiendas(): void {
    ...
    this.subscriptions.push(
      this.contiendasService.contiendasCompetenciasObservadasList(request).subscribe({
        next: resp => {
          if (resp.code === 0) {
            this.contiendasObservadasList = resp.data.map( (caso:any) => ({...caso, seleccionado: false}))
            this.contiendasObservadasFiltrados = this.contiendasObservadasList
            this.casosSeleccionados = []
            this.itemPaginado.data.data = this.contiendasObservadasFiltrados
            this.itemPaginado.data.total = this.totalCasosAsignar = this.contiendasObservadasList.length
            this.obtenerLeyenda(this.contiendasObservadasFiltrados)
            this.actualizarPaginaRegistros(this.contiendasObservadasList)
          }
        },
        error: error => {
          console.error(error)
  ...
  public eventoMostrarOcultarFiltros(): void {
    ...
    this.mostrarFiltros && this.buscarObservadosPorMeses()
  }

  public filtrarCasos(): void {
    let textoBusqueda = this.formularioFiltrarCasos.get('buscar')!.value;
    this.contiendasObservadasFiltrados = this.contiendasObservadasList.filter(item =>
    ...
    this.itemPaginado.data.data = this.contiendasObservadasFiltrados
    this.itemPaginado.data.total = this.totalCasosAsignar = this.contiendasObservadasFiltrados.length
    this.actualizarPaginaRegistros(this.contiendasObservadasFiltrados)
  ...
  public exportarPdfExcel(exportType: TipoArchivoType): void {
    if (this.contiendasObservadasList.length > 0) {
      ...
      this.contiendasObservadasList.forEach((caso: ContiendasCompetencias) => {
        const row = {
          ...
          'Fecha elevación al despacho': caso.fechaElevacion
        }
        data.push(row)
      })
  ...
  actualizarPaginaRegistros(data: any) {
    ...
    this.contiendasObservadasFiltrados = data.slice(start, end);
  }
