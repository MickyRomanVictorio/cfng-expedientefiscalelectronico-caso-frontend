...
@Component({
  selector: 'app-contienda-competencia',
  standalone: true,
...
export class ContiendaCompetenciaComponent implements OnInit {
  ...
  protected leyenda: PlazosLeyenda[] = PlazosLeyenda.filter(
    (l) => l.codigo !== '4'
  ); // No mostrar el registro de "Sin plazo"
  ...
  protected opcionesCaso: MenuItem[] = [
    ...
    },
    {
      ...
      command: () => this.eventoHistorialCaso(),
    },
    {
      label: 'Atender Caso',
  ...
  private leerCaso(datos: any): void {
    const caso = this.listaCasos.find(
      (caso: any) =>
        caso.idBandejaElevacion === this.casoSeleccionado.idBandejaElevacion
    );
  ...
  private obtenerDatosCasos(datos: any) {
    ...
    this.bandejaFiscaliaSuperiorService
      .obtenerContiendaCompetencia(datos)
      .subscribe({
        next: (resp: any) => {
