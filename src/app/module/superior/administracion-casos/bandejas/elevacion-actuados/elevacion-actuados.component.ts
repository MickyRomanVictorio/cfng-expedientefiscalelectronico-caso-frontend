...
import {
  AcumulacionesCasoComponent
} from '@core/components/consulta-casos/components/acumulaciones-caso/acumulaciones-caso.component';
...
@Component({
  selector: 'app-elevacion-actuados-superior',
  standalone: true,
  ...
  styleUrl: './elevacion-actuados.component.scss'
})
export class ElevacionActuadosComponent implements OnInit {
  ...
  protected leyenda: PlazosLeyenda[] = PlazosLeyenda.filter(l => l.codigo !== '4');
  ...
  private eventoVisorDocumental() {
    this.registarCasoLeido(this.casoSeleccionado.idCaso, this.casoSeleccionado.esLeido);
  ...
  protected eventoExportar(tipoExportar: TipoExportarEnum): void {
    if (this.listaCasosOriginal.length > 0) {
      ...
      tipoExportar === TipoExportarEnum.Pdf
        ? this.exportarService.exportarAPdf(data, headers, "bandeja_casos_actuados")
        : this.exportarService.exportarAExcel(data, headers, "bandeja_casos_actuados");
    }
  }
  ...
  private obtenerDatosCasos(datos: any) {
    ...
    this.bandejaFiscaliaSuperiorService.obtenerElevacionActuados(datos).subscribe({
      next: (resp: any) => {
  ...
  protected eventoVerDelitosPartes(e: Event, caso: any) {
    this.registarCasoLeido(caso.idCaso, caso.esLeido);
    e.stopPropagation();
  ...
  private registarCasoLeido(idCaso: String, leido: string): void {
    ...
    let request: CasoLeidoRequest = {
      numeroCaso: idCaso,
      idEstadoCaso: 0,// tabla de estados de caso el id 0 es SIN ESTADO
    };
