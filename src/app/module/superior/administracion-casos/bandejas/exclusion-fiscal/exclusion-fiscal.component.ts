...
import { AcumulacionesCasoComponent } from '@core/components/consulta-casos/components/acumulaciones-caso/acumulaciones-caso.component';
...
import { EtiquetaClasesCss, PlazosLeyenda, PlazosLeyendaClasesCss } from '@core/constants/superior';
...
@Component({
  selector: 'app-exclusion-fiscal-superior-bandejas',
  standalone: true,
  ...
  templateUrl: './exclusion-fiscal.component.html',
  styleUrl: './exclusion-fiscal.component.scss'
})
export class ExclusionFiscalComponent implements OnInit {
  ...
  protected tipoContienda: SelectItem[] = [
    {
      value: "988",
      label: "Contienda negativa"
    },
    {
      value: "987",
      label: "Contienda positiva"
    }
  ];
  ...
  protected leyendaClasesCss: { [key: string]: string } = PlazosLeyendaClasesCss;
  protected etiquetaClasesCss: { [key: string]: string } = EtiquetaClasesCss;
  protected leyenda: PlazosLeyenda[] = PlazosLeyenda.filter(l => l.codigo !== '4');// No mostrar el registro de "Sin plazo"
  ...
  protected opcionesCaso: MenuItem[] = [
    ...
    {
      label: 'Ver historial del caso', command: () => this.eventoHistorialCaso()
    },
    {
  ...
  /**
   * Indica si el caso ha sido leido
   * @param datos
   */
  private leerCaso(datos:any):void{
    const caso = this.listaCasos.find((caso: any) => caso.idBandejaElevacion === this.casoSeleccionado.idBandejaElevacion);
    if(caso && caso.esLeido === '0'){
      caso.esLeido='1';
      this.bandejaFiscaliaSuperiorService.leerCaso({
        idBandejaElevacion: this.casoSeleccionado.idBandejaElevacion
      }).subscribe({
        next: (response: any) => {},
        error: (error) => {}
      });
  ...
  protected eventoExportar(tipoExportar: TipoExportarEnum): void {
    if (this.listaCasosOriginal.length > 0) {
      ...
      tipoExportar === TipoExportarEnum.Pdf
        ? this.exportarService.exportarAPdf(data, headers, "bandeja_casos_exclusion_fiscal")
        : this.exportarService.exportarAExcel(data, headers, "bandeja_casos_exclusion_fiscal");
    }
  }
  ...
  private obtenerDatosCasos(datos: any) {
    ...
    this.bandejaFiscaliaSuperiorService.obtenerExclusionFiscal(datos).subscribe({
      next: (resp: any) => {
  ...
  protected eventoVerAcumulaciones(e: Event, caso: any) {
    e.stopPropagation();
    this.dialogService.open(AcumulacionesCasoComponent, {
      showHeader: false,
