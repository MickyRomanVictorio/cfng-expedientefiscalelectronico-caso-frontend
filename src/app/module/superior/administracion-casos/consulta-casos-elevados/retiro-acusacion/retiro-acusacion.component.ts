...
} from '@modules/superior/administracion-casos/consulta-casos-elevados/filtrar-consulta-caso/filtrar-consulta-caso.component';

@Component({
  standalone: true,
  selector: 'app-consulta-casos-elevados-retiro-acusacion',
...
export class RetiroAcusacionCasoElevadoComponent implements OnInit {
  ...
  private obtenerDatos(datos:any){
    this.casosElevadosService.obtenerCasos(TipoElevacionCodigo.RetiroAcusacion, datos).subscribe({
      next: (rs) => {
  ...
  private contarLeyendaPlazos(casos: CasoFiscal[]) {
   ...
   const leyenda = this.consultaCasosGestionService.getLeyendaPlazos().filter((item: any) => item.codigo !== '4');
  ...
  protected eventoExportar(tipoExportar: TipoExportar): void {
    if (this.listaCasos.length > 0) {
      ...
      tipoExportar === TipoExportar.Pdf? this.exportarService.exportarAPdf(data, headers, 'consultar_casos_retiro_acusacion') : this.exportarService.exportarAExcel(data, headers, 'consultar_casos_retiro_acusacion');
    }
  }
