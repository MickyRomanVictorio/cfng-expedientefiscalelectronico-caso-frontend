import { ComponentFixture, TestBed } from '@angular/core/testing';
import { ContiendaCompetenciaComponent } from './contienda-competencia.component';
import { provideHttpClient } from '@angular/common/http';

describe('ListarConsultaCasosComponent', () => {
  let component: ContiendaCompetenciaComponent;
  let fixture: ComponentFixture<ContiendaCompetenciaComponent>;

  beforeEach(async () => {
    const mockToken = JSON.stringify({token:'Bearer eyJhbGciOiJIUzUxMiJ9..XP8ZiMZSizPtvJ-3LLKxsAhKJYgR50A5jZU-Z9T1wv9SgDKzgag7R-27kG185HgdbDVIHDaJ73-bYfV_Mz2J3g'});
    spyOn(sessionStorage, 'getItem').and.callFake((key: string) => {
      return mockToken;
    });
    //
    await TestBed.configureTestingModule({
      imports: [
        ContiendaCompetenciaComponent
      ],
      providers: [
        provideHttpClient()
      ],
    })
    .compileComponents();

    fixture = TestBed.createComponent(ContiendaCompetenciaComponent);
    component = fixture.componentInstance;
    fixture.detectChanges();
  });

  it('should create', () => {
    expect(component).toBeTruthy();
  });
});
