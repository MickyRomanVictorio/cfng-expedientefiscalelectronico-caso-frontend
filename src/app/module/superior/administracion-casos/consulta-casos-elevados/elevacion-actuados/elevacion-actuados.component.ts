...
@Component({
  standalone: true,
  selector: 'app-consulta-casos-elevados-elevacion-actuados',
...
export class ElevacionActuadosComponent implements OnInit {
  ...
  private obtenerDatos(datos:any){
    this.casosElevadosService.obtenerCasos(TipoElevacionCodigo.ElevacionActuados, datos).subscribe({
      next: (rs) => {
  ...
  private contarLeyendaPlazos(casos: CasoFiscal[]) {
    ...
    console.log("los plazos son: ", leyenda);
  ...
  protected eventoExportar(tipoExportar: TipoExportar): void {
    if (this.listaCasos.length > 0) {
      ...
      tipoExportar === TipoExportar.Pdf? this.exportarService.exportarAPdf(data, headers, 'consultar_casos_elevación_actuados') : this.exportarService.exportarAExcel(data, headers, 'consultar_casos_elevación_actuados');
    }
  }
