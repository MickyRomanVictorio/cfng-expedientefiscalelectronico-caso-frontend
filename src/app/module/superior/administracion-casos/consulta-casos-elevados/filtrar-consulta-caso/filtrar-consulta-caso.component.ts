...
import {TipoVistaEnum} from '@components/consulta-casos/models/listar-casos.model';
import {ConsultaCasosGestionService} from '@components/consulta-casos/services/consulta-casos-gestion.service';
...
import {rangoFechaXDefecto} from '@utils/date';
...
export class FiltrarConsultaCasoComponent implements OnInit{
  ...
  constructor(
    ...
    this.plazos =this.consultaCasosGestionService.getLeyendaPlazos().map(leyenda => ({label:leyenda.nombre, value:leyenda.codigo})).filter(item => item.value !== '4');
  }
