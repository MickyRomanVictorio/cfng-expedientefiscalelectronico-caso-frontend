...
export class FiltrarConsultaCasoComponent implements OnInit{
  ...
  protected clasificacion: SelectItem[] = [
    ...
    {label: 'Cuaderno incidental', value: '2'}
  ];
  ...
  protected subTipoProceso = [
    { label: 'Todos', value: '0' },
    { label: 'Proceso Común', value: '1' },
    { label: 'Proceso Inmediato', value: '2' },
  ];
  protected cuadernoIncidental = [
    { label: 'Todos', value: '0' }
  ];
  protected fiscalProvincial = [];
  ...
  private listarTiposCuadernosIncidentales(): void {
    this.subscriptions.push(
      this.cuadernosIncidentalesService.listaClasificadorExpediente()
        .subscribe({
          next: (rs) => {
            this.cuadernoIncidental = rs.data
          }
        })
    )
  }
