...
import { IconAsset, IconUtil, ESTADO_REGISTRO } from 'ngx-cfng-core-lib';
...
import { ComboString } from "@interfaces/comunes/comboString";
...
import { SujetoCasoResultado } from "@interfaces/provincial/tramites/fundado-procedente/sujeto-caso-resultado";
...
@Component({
  selector: 'app-infundada-improcedente',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
  ...
  templateUrl: './infundada-improcedente.component.html',
  styleUrl: './infundada-improcedente.component.scss'
})
export class InfundadaImprocedenteComponent implements OnDestroy {
    ...
    this.subscriptions.push(
      this.apelacionesResultadosService
        .listarApelaciones(
          ...
          ID_N_TIPO_APELACION_SUJETO.PROCESO_INMEDIATO
        )
        .subscribe({
          next: (resp) => {
            if (resp?.code === 200) {
  ...
  validarMessageValidateFiscaliaSuperior(codigo: string): void {
    this.validateMessageService.cambiarValidacion(codigo === '' || codigo === null);
  }
