export const SYSTEM_CODE = '155';
export const SYSTEM_NAME = 'gen';
export const PRODUCTION = false;
export const ENV = 'qa';
export const VERSION = '1.4.0';
export const TIEMPO_INACTIVIDAD = 600000

export const DOMAIN_FRONT = 'http://localhost:4200';
export const DOMAIN_AUTH = 'http://172.16.111.112:8081';
export const DOMAIN_BACK = 'http://172.16.111.128:8081';
export const DOMAIN ='apps.ocp4.cfe.mpfn.gob.pe';
export const BASE_URL_GENERADOR = ''
export const DOMAIN_BACK_LOCAL_SUJETOS = 'http://localhost:8082';

export const IP_RENIEC = '201.240.68.38';

export const DOMAIN_FRONT_NOTIFICADOR = `http://cfng-generador-notificaciones-qa.apps.dev.ocp4.cfe.mpfn.gob.pe`;
export const DOMAIN_FRONT_HOME = 'http://cfng-home-qa.apps.ocp4.cfe.mpfn.gob.pe';
export const DOMAIN_ASSETS = 'http://172.16.111.112:8085/assets';
export const BACKEND = {

    CFE_REPORTES: `http://cfms-generales-reportes-api-qa.apps.ocp4.cfe.mpfn.gob.pe`,
    AFILIACION_CASILLA: `http://localhost:8080/ms-afiliacion-casilla`,
    CFEEXPEDIENTE: `http://cfms-expedientefiscal-electronico-gestion-api-qa.${DOMAIN}/cfe/expedientefiscalelectronico/`,
    CFETOKEN: `${DOMAIN_AUTH}/cfetoken/resources/v2/login`,
    CFEMAESTROS: `http://cfms-generales-maestros-gestion-api-qa.${DOMAIN}/cfe/generales/maestro/`,
    CFEPERSONA: `http://cfms-generales-persona-cliente-api-qa.${DOMAIN}/cfe/generales/persona`,
    CFENOTIFICACIONES: `http://cfms-notificaciones-generador-bf-gestion-api-qa.${DOMAIN}/cfe/generador/v1`,
    CFE_EFE : `http://cfms-expedientefiscal-electronico-gestion-api-qa.${DOMAIN}/cfe/expedientefiscalelectronico`,
    CFMFIRMADIGITAL: `http://cfms-generales-firmadigital-bf-gestion-api-qa.${DOMAIN}/cfe/generales/firmadigital/v1/t/firmadigital/`,
    CFEMAESTROSDOCUMENTOS: `http://cfms-generales-documentos-gestion-api-qa.${DOMAIN}/cfe/generales/documento/`,
    CFMREPOSITORIO: `http://cfms-generales-almacenamiento-objetos-api-qa.${DOMAIN}/cfe/generales/objetos/v1/t/almacenamiento/publico/`,
    CFETURNO: `http://cfms-mesa-de-turno-gestion-api-qa.${DOMAIN}/cfe/mesadeturno/v1`,


     /*MESA UNICA-DESPACHO*/
    MESA_UNICA_DESPACHO: `http://cfms-mesadepartes-unicadespacho-gestion-api-qa.${DOMAIN}/cfe/mesadepartes/unicadespacho/`,
    MS_PERSONA: `http://cfms-generales-persona-cliente-api-qa.${DOMAIN}/cfe/generales/persona/v1/`,
    MESA_DOCUMENTO: `http://cfms-mesadepartes-documentos-gestion-api-qa.${DOMAIN}/cfe/mesadepartes/documentos/`,
    FIRMA_CLIENTE: `http://cfms-generales-firmadigital-gestion-api-qa.${DOMAIN}/cfe/generales/firmadigital/v1/t/firmadigital/`,
    REPOSITORIO_DOCUMENTO: `http://cfms-generales-almacenamiento-objetos-api-qa.${DOMAIN}/cfe/generales/objetos/v1/t/almacenamiento/publico/`,
    REPOSITORIO_DOCUMENTO_ALFRESCO: `http://cfms-generales-almacenamiento-objetos-api-qa.${DOMAIN}/cfe/generales/objetos/v2/t/almacenamiento/publico/`,
    SUNAT: `http://cfms-generales-persona-cliente-api-qa.${DOMAIN}/cfe/generales/persona/v1/e/padronsunat/`,
    CFE_EFE_ALERTAS: `http://cfms-efe-alertas-gestion-api-qa.${DOMAIN}/cfe/expedientefiscalelectronico/alertas`,

    IDAU: `https://idau.mpfn.gob.pe:7000/api/cfe-2fa/`,
    NOTDOCUMENTOS: `http://cfms-notificaciones-documentos-bf-gestion-api-qa.apps.ocp4.cfe.mpfn.gob.pe/cfe/generador/documentos/v1/repositorio`,
    CASILLA_ELECTRONICA: 'http://cfms-integracion-casilla-api-qa.apps.ocp4.cfe.mpfn.gob.pe/ms-integracion-casilla/',

    CFE_GESTOR_DOCUMENTAL: `http://cfms-generales-gestor-documental-api-qa.${DOMAIN}/cfe/generales/gestordocumental`,
    CFE_EFE_TRAMITES : ` http://cfms-efe-tramite-gestion-api-qa.${DOMAIN}/cfe/expedientefiscalelectronico/tramite`,

    CFE_EFE_SUJETOS : `http://cfms-efe-sujeto-gestion-api-qa.${DOMAIN}/cfe/expedientefiscalelectronico/sujeto`,
    CFE_EFE_DERIVACIONES: `http://cfms-efe-derivacion-gestion-api-qa.${DOMAIN}/cfe/expedientefiscalelectronico/derivacion`,
    CFE_EFE_CUADERNO: `http://cfms-efe-cuaderno-gestion-api-qa.${DOMAIN}/cfe/expedientefiscalelectronico/cuaderno`,
    REPOSITORIO_DOCUMENTO_PRIVADO: `http://cfms-generales-almacenamiento-objetos-api-qa.${DOMAIN}/cfe/generales/objetos/v2/t/almacenamiento/privado`,
    CFE_EFE_SUPERIOR : `http://cfms-efe-superior-gestion-api-qa.${DOMAIN}/cfe/expedientefiscalelectronico/superior`,
    CFE_EFE_REPOSITORIO_PUBLICO: `http://cfms-generales-repositorio-publico-api-qa.${DOMAIN}`,

    CFEAPLICACIONES: `http://cfms-sad-administracion-gestion-api-qa.${DOMAIN}/cfe/sad/administracion/v1/e/usuario/listarAplicaciones/`,
    CFEOPCIONES: `http://cfms-sad-administracion-gestion-api-qa.${DOMAIN}/cfe/sad/administracion/v1/e/usuario/listarOpcionesByUsuarioApliacion/`
};



export const CRYPT_KEY = 'a1b2c3z28';

export const LOCALSTORAGE = {
    TOKEN_KEY: 'dG9rZW4=',
    REFRESH_KEY: 'cmVmcmVzaA==',
    VALIDATE_KEY: 'dmFsaWRhdGlvbg==',
    DENUNCIA_KEY: 'ZGVudW5jaWE=',
    PRECARGO_KEY: 'cHJlY2FyZ28=',
    DELITO_KEY: 'ZGVudW5jaWE=',
    NOMBRE_DOCUMENTO_KEY: 'bm9tYnJlIGRvY3VtZW50bw==',
    CASO_KEY: 'aWQgY2Fzbw==',
    ETAPA_KEY: 'Y2FzbyBldGFwYQ==',
    CASO_OBJETO_KEY: 'Y2FzbyBvYmpldG8=',
    TAB_DETALLE_SELECCIONADO_KEY: 'dGFiIGRldGFsbGUgc2VsZWNjaW9uYWRv',
    TRAMITE_RSP_KEY: 'xewk5Xpc9TU=',
};

export const UTILS = {
    DEFAULT_PHOTO: 'data:image/png;base64,'
}
export const TAMANIO_EN_MEGABYTES: number = 100;
export const TAMANIO_MAXIMO_ARCHIVO_AUDIOS :number = TAMANIO_EN_MEGABYTES * 1024 * 1024; // 100MB en bytes

